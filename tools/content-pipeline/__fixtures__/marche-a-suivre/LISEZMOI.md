...
| `02-guide` | `brouillon` | porte le `:::: marche-a-suivre`, ses six étapes, ses quatre renvois et ses trois voies |
...
4. `{voie="cours"}` **seul** — la voie sans renvoi (décision D-PHP-1, lot PHP-A1).
5. `{voie="…"}` **puis** `{voir="…"}` — la tête porte deux blocs, dans cet ordre-là.
6. `{voir="…"}` **puis** `{voie="…"}` — le même couple dans l'ordre INVERSE. Les deux ordres sont
   exercés parce que le contrat les admet tous les deux : un seul cas laisserait la moitié de la
   boucle de lecture de tête sans exercice.
