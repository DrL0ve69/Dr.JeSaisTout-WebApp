...
# CONTRÔLE POSITIF DE L’EXEMPTION, placé AVANT la faute EXPRÈS : un marqueur ⚠ dans un bloc
# de code est LÉGAL. Une leçon a le droit de citer une fiche KB verbatim, ou d’enseigner la
# notation 📘/🧩/⚠ elle-même — ce sont des données, pas du balisage. Ce ⚠-ci est la forme NUE
# (U+26A0 seule), la faute d’en bas porte la forme ÉMOJI (U+26A0 U+FE0F) : les deux se refusent
# hors code par le MÊME contrôle, et se tolèrent ici par la MÊME exemption. Si l’analyse par
# tranches se cassait, c’est CETTE ligne-ci qui serait rapportée en premier, et le spec le verrait.
echo 'la langue de la cloture appartient a la liste fermee du contrat'
```

## À toi de jouer

LA FAUTE EST ICI : ⚠️ employé en PROSE pour accuser le cours d'être faux, là où la correction
doit passer par un encadré `::: correction-du-cours {source="…"}` — le seul chemin qui rende la
source OBLIGATOIRE, donc la relecture possible. Le quiz vit dans `quiz.json`. Le marqueur dit au
