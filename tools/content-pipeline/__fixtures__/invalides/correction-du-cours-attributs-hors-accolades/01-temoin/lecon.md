...
LA FAUTE EST À LA LIGNE D’OUVERTURE CI-DESSUS : la source est bien là, mais SANS ACCOLADES.
`lireAttributs` (compiler-markdown.mjs) impose la forme `{clef="valeur"}` ; avant le 2026-08-20,
le motif de G3 cherchait `source="…"` n’importe où dans la suite, donc cette ligne PASSAIT le
validateur et faisait échouer le COMPILATEUR — sur « seule la forme {clef="valeur"} est
acceptée », un message qui n’a rien à voir avec ce que l’auteur croit avoir écrit.
:::
