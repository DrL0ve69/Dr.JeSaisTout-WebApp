...
# notation 📘/🧩 elle-même — ce sont des données, pas du balisage. Si l’analyse par tranches
# se cassait, c’est CETTE ligne-ci qui serait rapportée en premier, et le spec le verrait.
echo 'la langue de la cloture appartient a la liste fermee du contrat'
```

## À toi de jouer

LA FAUTE EST ICI : 🧩 employé en PROSE, là où la provenance doit passer par un encadré
`::: complement`. Le quiz de cette leçon vit dans `quiz.json`. Le marqueur ci-dessous dit au
